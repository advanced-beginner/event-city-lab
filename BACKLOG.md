...
- 완료기준:
  ...
  - [ ] 테스트: 같은 experiment의 choice들은 preview의 nodeIds 또는 routeIds가 서로 다르다
  - [ ] 기존 검사(모든 id가 scene에 존재) 유지
  - [ ] 시각 변경 의도: 실행 전 preview만. baseline(initial 상태 캡처)이 바뀌는 챕터는 이력에 적고 갱신한다
